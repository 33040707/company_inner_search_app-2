...
# ==========================================
MODEL = "gpt-4o-mini"
TEMPERATURE = 0.2  # 単価表などの正確な抽出のため低めに設定
CHUNK_SIZE = 1000  # 表形式データが途切れないよう少し大きめに拡張
CHUNK_OVERLAP = 100
TOP_K = 8          # より多くの関連ドキュメント（ページ）を取得
...
SYSTEM_PROMPT_INQUIRY = """
    ...
    以下の【文脈】に含まれる情報を抽出し、ユーザーからの質問に対してわかりやすく回答してください。

    【条件】
    1. 【文脈】の中に質問に関連する内容（価格表、職種ごとの単価、数値、規定等）が含まれている場合は、該当箇所を抽出し、表や箇条書きを用いてわかりやすく回答してください。
    2. ユーザーが特定の単価や数値を求めている場合は、【文脈】に書かれている情報を可能な限り具体的に書き出してください。
    3. 【文脈】の中に質問に関連する情報が一切含まれていない場合のみ、「回答に必要な情報が見つかりませんでした。」と回答してください。
    4. マークダウン記法で回答する際に見出しを使う場合、最も大きい見出しをh3としてください。
